...
void Dialog_AddPick::setType(bool _isAdd,QMap<QString,QString>* _StuMap,QMap<QString,QString>* _CourseMap,PickInfo _in)
{
    StuMap = *_StuMap;
...
void Dialog_AddPick::on_pushButton_save_clicked()
{
    ...
    QString feedback;
    if(isAdd){
        if(!info.CNo.isEmpty() && SqlPtr->addPick(info)) feedback = "成功添加学号为\'"+info.SNo+"\'课程号为\'"+info.CNo+"\'的选课信息";
        else feedback = "添加失败";
